...
超时空会夜机是一款支持**异地同步观影**的桌面应用。朋友间各自打开客户端，进入同一个房间后播放进度就会实时同步。支持本地视频文件播放，也支持直接粘贴 B站 视频链接在线观看。

核心同步机制：房主的播放操作（播放/暂停/进度跳转）通过 WebSocket 实时广播给房间内所有成员，每 3 秒还会自动校正时间差，确保所有人的画面保持一致。
...
注：已上传服务端和客户端的打包文件至 `E:\Study\超时空会夜机\`，可直接下载使用。仅需**一人**配置服务端，其他人下载客户端即可。

### 环境要求

- Python 3.8+
- [ngrok](https://ngrok.com/) 或其他内网穿透工具（服务端需要）
- [libmpv](https://mpv.io/installation/)（视频播放运行时，已内置在项目中）
